...
class NoDataResponse(Exception):
    def __init__(self, message, date_insert):
        super().__init__(message)
        self.date_insert = date_insert

    

class DateAlreadyProcessed(Exception):
    def __init__(self, message, date_insert):
        super().__init__(message)
        self.date_insert = date_insert
...
def transform_load_OHCL():
    try:
        ...
        if len(loaded_time_df) >= 1:
            raise DateAlreadyProcessed("DATE PROCESSED", et_time.date())
            return
        ...
        if len(df) < 1:
            ...
            raise NoDataResponse("NO DATA RESPONSE", et_time.date() )
            return
    ...
    except DateAlreadyProcessed as e:
        print(f"Du lieu da duoc xu ly: {e.date_insert}")
    except NoDataResponse as e:
        print(f"Du lieu da duoc xu ly: {e.date_insert}")
        # xu ly exception khong co du lieu tra ve
