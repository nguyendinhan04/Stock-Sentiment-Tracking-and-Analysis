...
def load_news_hourly(**kwargs):
    ...
    clickhouseClient = getClickhouseClient()
    lastest_obj = None
    obj_list = minioClient.list_objects(bucket_name="news-data-hourly")
    for item in obj_list:
        if not lastest_obj or item.last_modified > lastest_obj.last_modified :
            lastest_obj = item
    if not lastest_obj:
        return
    # test_object_name = "news_2025-03-31.json"
    et_time = datetime.strptime(lastest_obj.object_name[5:20] + "00", "%Y-%m-%dT%H%M%S")
    # et_time = datetime.strptime(test_object_name[5:15], "%Y-%m-%d")
    


    # time_now = datetime.strptime(execution_date, r"%Y-%m-%d")
    # time_now = et_time + timedelta(days = 3)
    # print(et_time.date())
    # print("now: " + str(time_now))
    ...
    # insert news
    insert_news_df = df[["title","url", "time_published","source","overall_sentiment_score","in_time_id"]]
    current_source_df = get_table("source_DIM")
    join_news_df = pd.merge(insert_news_df,current_source_df,left_on = "source",right_on = "source_name",suffixes = ["_left","_right"],how = "left")
    join_news_df["time_published"] = join_news_df["time_published"].apply(lambda x : str(datetime.strptime(x, "%Y%m%dT%H%M%S")))

    join_news_df["insert_time"] = str(et_time)
    join_news_df.rename(columns = {"in_time_id" : "in_time_id"},inplace = True)
    # print(join_news_df.info())

    if(len(insert_news_df) >= 1):
        clickhouseClient.insert_dataframe(query = "insert into news_DIM_hourly(title, url, time_publish_id, source_id, overall_sentiment_score,insert_time,in_time_id) values", dataframe = join_news_df[["title","url","time_published", "source_id","overall_sentiment_score","insert_time","in_time_id"]].rename(columns = {
            "time_published": "time_publish_id",
        }))
    ...
    # insert sentiment topic FACT
    sentiment_industry_df = df[["in_time_id", "topics"]]
    sentiment_industry_df = sentiment_industry_df.explode("topics")
    sentiment_industry_df = sentiment_industry_df[pd.notna(sentiment_industry_df["topics"])]
    sentiment_industry_df["topic"] = sentiment_industry_df["topics"].apply(lambda x : x.get("topic"))
    sentiment_industry_df["sentiment_score"] = sentiment_industry_df["topics"].apply(lambda x : x.get("relevance_score"))
    ...
    insert_sentiment_industry_df = join_sentiment_industry_df[["in_time_id","topic_id","sentiment_score"]]
    insert_sentiment_industry_df["insert_time"] = str(et_time)
    # print(insert_sentiment_industry_df.info())

    clickhouseClient.insert_dataframe(query = "insert into new_sentiment_industry_hourly_FACT(insert_time,in_time_id, topic_id, sentiment_score) values",dataframe = insert_sentiment_industry_df)

    # insert sentiment stock FACT   
    sentiment_stock_df = df[["in_time_id", "ticker_sentiment"]]
    sentiment_stock_df = sentiment_stock_df.explode("ticker_sentiment")
    sentiment_stock_df = sentiment_stock_df[pd.notna(sentiment_stock_df["ticker_sentiment"])]
    sentiment_stock_df["ticker"] = sentiment_stock_df["ticker_sentiment"].apply(lambda x : x.get("ticker"))
    sentiment_stock_df["sentiment_score"] = sentiment_stock_df["ticker_sentiment"].apply(lambda x : x.get("ticker_sentiment_score"))
    ...
    insert_sentiment_stock_df = join_sentiment_stock_df[["in_time_id","stock_symbol","sentiment_score"]]
    insert_sentiment_stock_df["insert_time"] = str(et_time)

    clickhouseClient.insert_dataframe(query = "insert into new_sentiment_stock_hourly_FACT(insert_time, in_time_id, stock_symbol, sentiment_score) values", dataframe = insert_sentiment_stock_df)
